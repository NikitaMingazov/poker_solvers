...
#[derive(Debug, Clone, PartialEq, Copy, Hash, Eq, serde::Deserialize)]
...
impl serde::Serialize for Hand {
	...
	where
		S: serde::Serializer,
	{
		let card_strings: Vec<String> = self.0.into_iter()
			.map(|card| card.to_string())
			.collect();
		let hand_str = card_strings.join(" ");
		serializer.serialize_str(&hand_str)
	}
}
/*
...
impl<'de> serde::Deserialize<'de> for Hand {
	...
	{
		deserializer.deserialize_seq(CardBitSet)
	}
}
*/
